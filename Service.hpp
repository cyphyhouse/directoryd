...
public:
    Service(string const &type, unsigned short port, map<string, string> const &txt) : port_(port), txt_(txt), type_(type) {}

    Service(string const &type, string const &hostname, unsigned short port, AvahiStringList *txt) : port_(port), hostname_(hostname), type_(type)
    {
        parse_txt(txt);
    }

    Service(string const &type, string const &hostname, const AvahiAddress *address, unsigned short port, AvahiStringList *txt)

...
public:
	explicit Browser (string const &type) : type_(type) {
		Avahi::instance().lock();
		sb_ = avahi_service_browser_new(Avahi::instance().client(), AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, type_.c_str(), NULL,
	...
	const std::vector<Service> lookup_by_name (std::string const &name) const {
		...
		std::for_each(result.first, result.second, [&found](decltype(*result.first) &elt) {
	...
	const std::vector<Service> lookup_by_name (std::string const &name, Predicate const &pred) const {
		std::vector<Service> found;
		Avahi::instance().lock();
		auto result = services.equal_range(name);
		auto s = std::find_if(result.first, result.second, [&pred](std::pair<std::string, Service> const &x) {
			return pred(x.second);
		});
		while (s != result.second) {
			found.push_back(s->second);
			s++;
			s = std::find_if(s, result.second, [&pred](std::pair<std::string, Service> const &x) {
				return pred(x.second);
			});
		}
		Avahi::instance().unlock();
	...
	const std::vector<Service> lookup (Predicate const &pred) const {
		std::vector<Service> found;
		Avahi::instance().lock();
		auto s = std::find_if(services.begin(), services.end(), [&pred](std::pair<std::string, Service> const &x) {
			return pred(x.second);
		});
		while (s != services.end()) {
			found.push_back(s->second);
			s++;
			s = std::find_if(s, services.end(), [&pred](std::pair<std::string, Service> const &x) {
				return pred(x.second);
			});
		}
		Avahi::instance().unlock();

...
void reply(zmq::socket_t &socket, vector<string> const &header, directoryd::RequestType type,
...
void reply_result(zmq::socket_t &socket, vector<string> const &header, directoryd::RequestType type,
		std::vector<Avahi::Service> const &findresult) {
	reply(socket, header, type, true, std::string(), findresult);
}

void reply_success(zmq::socket_t &socket, vector<string> const &header, directoryd::RequestType type,
		std::string const &result = std::string()) {
	reply(socket, header, type, true, result);
}

void reply_failure(zmq::socket_t &socket, vector<string> const &header, directoryd::RequestType type, std::string const &result) {
...
int main(int argc, char *const argv[]) {
	...
	while (true) {
		...
		zmq::context_t ctx(1);
		zmq::socket_t socket(ctx, ZMQ_XREP);
		socket.bind("ipc:///tmp/directoryd");
